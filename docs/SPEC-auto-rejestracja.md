...
### 2.3 Blokada klienta (isActive)

Tabela `clients` ma pole `isActive`. Klient z `isActive = false` jest
traktowany jako ZABLOKOWANY w aplikacji klienckiej:

- `request-code`: jeśli numer pasuje w tenancie wyłącznie do klientów
  nieaktywnych → **NIE wysyłać SMS-a** i **nie zapisywać kodu**, ale zwrócić
  identyczną odpowiedź 200 jak zwykle (zablokowany nie dostaje informacji,
  że jest zablokowany; nie zużywamy SMS-ów z puli tenanta).
- Taki numer NIE przechodzi też ścieżką auto-rejestracji (nie tworzymy
  duplikatu obok zablokowanego konta).
- `verify`: do `clientIds` w tokenie trafiają wyłącznie klienci z
  `isActive = true`. Jeśli część kont jest nieaktywna, a część aktywna —
  logowanie działa tylko dla aktywnych.
- Jeśli obecna implementacja client-auth już filtruje po `isActive` —
  zweryfikować i uzupełnić brakujące miejsca; jeśli nie filtruje — dodać.
...
6. request-code dla numeru pasującego tylko do klienta `isActive = false`
   → 200, ale SMS NIE wysłany i kod NIE zapisany; verify dla tego numeru
   nie może się powieść.
7. Kompilacja TypeScript przechodzi.
8. Praca wypchnięta na gałąź `claude/client-auth-autoreg`; w raporcie podać
