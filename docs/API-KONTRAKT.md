...
2. **Logowanie konsumenta + endpointy self-service** — **NIE ISTNIEJE.** Booksero nie ma logowania klienta; SMS-2FA i Google to logowanie **personelu**. Klient widzi dziś tylko **pojedynczą wizytę po tokenie** z linku. **„Moje wizyty" (historia) i „Lojalność" nie mają publicznego API** — punkty/historia są tylko pod `/api/salon/*` z JWT pracownika. → Wymaga zbudowania: (a) logowania klienta (kod SMS), (b) endpointów „moje wizyty / moja lojalność". (auth.ts:45-61; routes.ts:8539-8703)
